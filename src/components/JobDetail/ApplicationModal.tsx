...
type Props = { open: boolean; title: string; slug: string; salaryCurrency: string; onClose: () => void; onSubmitted: () => void };

export function ApplicationModal({ open, title, slug, salaryCurrency, onClose, onSubmitted }: Props) {
  ...
  return createPortal(<div className="apply-modal" role="dialog" aria-modal="true" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <form className={step === "profile" ? "apply-profile-step" : ""} onSubmit={submit}>
      ...
      {step === "application" ? checkingSubscription ? <p>Checking your CV options…</p> : choosingCv ? <>
        ...
        <label htmlFor="salary">Expected monthly salary ({salaryCurrency}) <small>optional</small></label>
