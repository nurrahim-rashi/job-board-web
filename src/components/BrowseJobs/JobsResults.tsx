              ...
              <Reveal key={job.slug} delay={Math.min(index, 6) * 70}>
                <article className="browse-job-card">
                  <div className="browse-card-top">
                    ...
                    <button
                      type="button"
                      aria-label={`${saved.includes(job.slug) ? "Unsave" : "Save"} ${job.title}`}
                  ...
                  <h2>{job.title}</h2>
                  <p className="browse-company">{job.company}</p>
                  <p className="browse-location">
                    <MapPin />
                    {job.city}
                    {distances[job.slug] != null
                      ? ` · ${distances[job.slug].toFixed(0)} km`
                      : ""}
                  </p>
                  <div className="browse-tags">
                    {[job.type, job.salary, job.category].map((tag) => (
                      <span key={tag}>{tag}</span>
                    ))}
                  </div>
                  <small>{relativePosted(job.postedAt)}</small>
