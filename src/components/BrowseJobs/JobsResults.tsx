...
export function JobsResults({ jobs, loading, onReset }: { jobs: PublicJob[]; loading: boolean; onReset: () => void }) {
  ...
  return <section className="browse-results"><div><p className="browse-count">{jobs.length} role{jobs.length === 1 ? "" : "s"} found</p>{jobs.length ? <div className="browse-job-grid">{jobs.map((job, index) => <Reveal key={job.slug} delay={Math.min(index, 6) * 70}><article className="browse-job-card"><div className="browse-card-top"><span>{job.company.companyName[0]}</span><button type="button" aria-label={`Save ${job.title}`} onClick={() => setSaved((current) => current.includes(job.slug) ? current.filter((slug) => slug !== job.slug) : [...current, job.slug])} className={saved.includes(job.slug) ? "saved" : ""}><Bookmark /></button></div><h2>{job.title}</h2><p className="browse-company">{job.company.companyName}</p><p className="browse-location"><MapPin />{job.cityLocation}{job.distance != null ? ` · ${job.distance.toFixed(0)} km` : ""}</p><div className="browse-tags"><span>{job.category.replaceAll("_", " ")}</span><span>{salary(job)}</span></div><small>{posted(job.createdAt)}</small><a href={`/jobs/${job.slug}`}>View role <ArrowRight /></a></article></Reveal>)}</div> : <div className="browse-empty"><h2>No roles match those filters.</h2><button type="button" className="button button-primary" onClick={onReset}>Clear filters</button></div>}</div></section>;
}
