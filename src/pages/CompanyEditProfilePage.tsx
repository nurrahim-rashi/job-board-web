      ...
      <div className="admin-profile-content">
        <section className="profile-card company-media-editor">
          ...
          <div className="profile-fields">
            <div className="company-media-field"><label>Company logo<input type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/avif,image/heic,image/heif,.jpg,.jpeg,.png,.webp,.gif,.avif,.heic,.heif" onChange={(event) => { const input = event.currentTarget; void saveMedia("logo", input.files?.[0]).finally(() => { input.value = ""; }); }} /></label>{company.logo && <button className="file-remove" type="button" onClick={() => void clearMedia("logo")} aria-label="Delete company logo" title="Delete picture">×</button>}</div>
            <div className="company-media-field"><label>Hero banner<input type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/avif,image/heic,image/heif,.jpg,.jpeg,.png,.webp,.gif,.avif,.heic,.heif" onChange={(event) => { const input = event.currentTarget; void saveMedia("banner", input.files?.[0]).finally(() => { input.value = ""; }); }} /></label>{company.banner && <button className="file-remove" type="button" onClick={() => void clearMedia("banner")} aria-label="Delete company banner" title="Delete picture">×</button>}</div>
          </div>
        </section>
