      ...
      <div className="admin-profile-content">
        <section className="profile-card company-media-editor">
          ...
          <div className="profile-fields">
            <div className="company-media-field"><label htmlFor="company-logo">Company logo</label><div className="company-media-row"><input id="company-logo" type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/avif,image/heic,image/heif,.jpg,.jpeg,.png,.webp,.gif,.avif,.heic,.heif" onChange={(event) => { const input = event.currentTarget; void saveMedia("logo", input.files?.[0]).finally(() => { input.value = ""; }); }} />{company.logo && <button className="file-remove" type="button" onClick={() => void clearMedia("logo")} aria-label="Delete company logo" title="Delete picture">×</button>}</div></div>
            <div className="company-media-field"><label htmlFor="company-banner">Hero banner</label><div className="company-media-row"><input id="company-banner" type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/avif,image/heic,image/heif,.jpg,.jpeg,.png,.webp,.gif,.avif,.heic,.heif" onChange={(event) => { const input = event.currentTarget; void saveMedia("banner", input.files?.[0]).finally(() => { input.value = ""; }); }} />{company.banner && <button className="file-remove" type="button" onClick={() => void clearMedia("banner")} aria-label="Delete company banner" title="Delete picture">×</button>}</div></div>
          </div>
        </section>
