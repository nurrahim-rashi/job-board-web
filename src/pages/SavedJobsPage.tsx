...
export default function SavedJobsPage() {
  ...
  return <div className="workspace-dashboard seeker-dashboard-overview"><Navbar /><main>
    <section className="workspace-hero seeker-list-hero"><div className="night-sky" /><Stars /><div className="workspace-hero-inner"><p className="eyebrow light">Your shortlist</p><h1>Saved jobs</h1><p>Keep promising opportunities close and return when you are ready to apply.</p></div></section>
    <SeekerDashboardShell><section className="seeker-application-page-card">
      ...
      {loading ? <div className="seeker-menu-empty"><h3>Loading saved jobs…</h3></div> : visibleJobs.length ? <div className="saved-job-list">{visibleJobs.map((job) => <article key={job.slug}><a href={`/jobs/${job.slug}`}><span><b>{job.title}</b><small>{job.company.companyName}</small><small><MapPin />{job.cityLocation} · {categoryLabel(job.category)}</small></span></a><button type="button" onClick={() => removeSaved(job.slug)}><Bookmark />Remove</button></article>)}</div> : <div className="seeker-menu-empty"><Bookmark /><h3>No saved jobs yet.</h3><p>Save a role from its job detail page and it will appear here.</p><a className="admin-btn" href="/jobs">Browse jobs</a></div>}
