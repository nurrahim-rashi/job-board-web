...
export type PublicCompanyDetail = Omit<PublicCompany, "_count"> & { phone: string; tagline: string; size: string; founded: number | null; website: string; products: { name: string; url: string; description: string }[] | null; values: string[]; perks: string[]; jobPostings: { id: number; slug: string; title: string; cityLocation: string; category: string; salaryMin: number | null; salaryMax: number | null; createdAt: string; deadline: string }[] };
